 ...
 * Ramsha — ProgressBar
 *
 * 3-step progress indicator for the Event Creation Flow.
 * Steps: Event Type -> Details -> Publish
 */
const ProgressBar = ({ currentStep }) => {
  const { t } = useLanguage();
  const steps = [
    { number: 1, label: t("stepEventType") },
    { number: 2, label: t("stepDetails") },
    { number: 3, label: t("stepPublish") },
  ];
  ...
  return (
    <div className="w-full bg-secondary-background border-b border-border py-4 px-6 md:px-12">
      ...
      <div className="md:hidden text-center mt-3 font-mono text-xs text-muted-foreground">
        {t("stepLabel")} {currentStep}: <span className="text-foreground font-bold uppercase">{steps[currentStep - 1]?.label}</span>
      </div>
    </div>
  );
};
