...
export default function WorkbackScheduleStep({ data, onChange, onNext, onBack }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-black text-foreground">{t("planningTitle")}</h2>
      ...
      <div className="rounded-base border-2 border-border bg-card p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-bold text-foreground">{t("autoGenerate")}</p>
            <p className="text-sm text-muted-foreground">
              {hasScheduleDates
                ? t("autoGenerateDesc")
                : t("autoGenerateNoDate")}
            </p>
          </div>
          <Button
            variant="neutral"
      ...
      <div className="rounded-base border-2 border-dashed border-border p-4 space-y-3">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="space-y-1">
            ...
            >
              <option value="pre-event">{t("preEvent")}</option>
              <option value="during">{t("duringEvent")}</option>
              <option value="post-event">{t("postEvent")}</option>
            </select>
          </div>
        </div>
        <Button
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext}>{t("nextResources")}</Button>
      </div>
    </div>
  );
}
